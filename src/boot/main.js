...
function autoFitToViewport(padding=20){
    ...
    const v=Math.round(state.scale*100); const sl=document.getElementById('scale'); const sv=document.getElementById('scaleVal');
...
// ---- Basics ----
function wireBasics(){
    themeToggle(); centeredZoom(newScale=>{ state.scale=newScale; render();
        if (window.TOOL_STAGE) window.TOOL_STAGE.validSelection(); });
